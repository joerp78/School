...
  I2C_start(OLED_ADDR);                   // start transmission to OLED
  I2C_write(OLED_DAT_MODE);               // set data mode
  for(uint16_t i=1024; i; i--) I2C_write(0x00); // clear the screen
  I2C_stop();                             // stop transmission
...
// Send a command byte to the OLED -- Control Brightness
void OLED_command(uint8_t cmd) {
  I2C_start(OLED_ADDR);     // Begin I2C transmission to OLED
  I2C_write(OLED_CMD_MODE); // Set to command mode
